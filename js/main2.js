...
function updateBarChart() {
  ...
  if(selectedCourseArray.indexOf("3")>-1 ||
	...
	if(turnA>=5){
	  ...
	  //document.getElementById("canvas_IF_line").style.display="";
	  //document.getElementById("canvas_IF_log").style.display="";
	  //document.getElementById("canvas_Tone_line").style.display="";
	  //document.getElementById("canvas_Tone_log").style.display="";
	  var fileStr = dirPath+treePath+tenFifteenPath+turnPath+bPath;
	  //var fileStr = 'algorithms/corpus/random_policy_result/tree3_4/10turns_after/after10_11-20/random_b';
	  ...
	  //oldToneScore = userScoreLog[0].slice(0);
	  ...
	  var googleDataIF = [],googleDataTone = [];
	  googleDataIF.push(['Unit','Score','Retroflex Score','Percentage']);
	  googleDataTone.push(['Unit','Score','Percentage']);
    	...
    	googleDataIF.push(tmpArray);
  	  }
  	  var googleDataIF2 = google.visualization.arrayToDataTable(googleDataIF);
  	  for(var i=0;i<Tone.length;i++){
		var tmpArray=[];
    	tmpArray.push(Tone[i]);
		tmpArray.push(oldToneScore[i]);
		tmpArray.push(printableCountTone[i]);
    	googleDataTone.push(tmpArray);
  	  }
		console.debug("rNum"+rNum);
		console.debug("IF"+printableCountIF);
		console.debug("Tone"+printableCountTone);
  	  var googleDataTone2 = google.visualization.arrayToDataTable(googleDataTone);

      // Create and draw the visualization.
 	  var optionIF = {
		title : 'Statistics Result using Proposed Policy',
		width: 1500,
		height: 300,
		animation: {duration:500},
		vAxes: {0:{title: "Average Score"},1:{title:"Percentage of Extra Practice over Random",format:'#%'}},
		hAxis: {title: "Acoustic Units(Initials/Finals)"},
		seriesType: "bars",
		series: {0:{targetAxisIndex:0,color:'Gainsboro'},1:{targetAxisIndex:0,color:'red'},2:{targetAxisIndex:1,type: "line"}}
  	  };
  	  var compareChartIF = new google.visualization.ComboChart(document.getElementById('RandomCompareIF'));
  	  compareChartIF.draw(googleDataIF2,optionIF);
 	  var optionTone = {
		title : 'Statistics Result using Proposed Policy',
		width: 1500,
		height: 300,
		...
		seriesType: "bars",
		series: {0:{targetAxisIndex:0,color:'Gainsboro'},1:{targetAxisIndex:1,type: "line"}}
  	  };
  	  var compareChartTone = new google.visualization.ComboChart(document.getElementById('RandomCompareTone'));
  	  compareChartTone.draw(googleDataTone2,optionTone);
	}
  }
  var lineChartDataIF={
labels : IF,
...
function onSuccessReturnScoreAndSentece(res) {
  ...
  if(detect2[0].indexOf("Analyze") != -1){  //analyze failed
	 ...
	 $('#selectionBox').empty();
	 $('#selectionBox').append(BUSC[0]);
	 $('#selectionBox').append(BUSC[1]);
	...
	for (var i=0;i<6;i++){
	  ...
	  if(i==0){
		...
		for (var j=0;j<separateScore.length;j++){
		  ...
		  tableContent += d[2*j]+'</span>';
		  tableContent += separateScore[j][1];
		  tableContent += '</td>';
		}
	  }
	  else if(i==2){
		tableContent += '<td>Final</td>';
		for (var j=0;j<separateScore.length;j++){
		  tableContent += '<td>';
		  tableContent += '<span class="label label-fixed">'+d[2*j+1]+'</span>';
		...
		if(i==3)
		  tableContent += '<td>Tone</td>';
		if(i==4)
		  tableContent += '<td>Timing</td>';
		if(i==5)
		  tableContent += '<td>Emphasis</td>';
		for (var j=0;j<separateScore.length;j++){
		  tableContent += '<td>';
		  tableContent += separateScore[j][i-(6-separateScore[j].length)];
		  tableContent += '</td>';
...
function playSample( e ){
  var i = $(".sentenceSelect:checked").val();
  var index = humanSentenceChoice[i];
  if(index!=undefined){
...
function onSuccess(res) {
  ...
  if(res.Computer[0].length==2){
	...
	$("<div class='bubbledLeft playMedia' onclick='imagePlay("+res.dialogueIndex+")'>"+ res.Computer[1]+ "</div><audio id='sentenceAudio" +res.dialogueIndex +"' src='dialogueTree_wav/" + res.dialogueIndex + ".wav' autoplay> </audio>").appendTo("#dialogueContentSection");
  ...
  if(res.recommendedIndex=='end'){
	...
	for (var i=0;i<res.Human.length;i++){
	  if (res.HumanSentenceID[i] == res.recommendedIndex){
		isRec=0;
		selectionBoxContent[isRec] += '<img src="img/star.png" width="45" height="30" align="left">';
	  ...
	  for(var j=0;j<res.Human[i][1].length;j++){
	  ...
	  for(var j=1;j<d.length;j+=2){
		if(d[j].charCodeAt(0)>=65 && d[j].charCodeAt(0)<=122)
		  selectionBoxContent[isRec] += '<td><span class="label label-fixed">'+d[j];
		else
		  selectionBoxContent[isRec] += '<td><span class="label label-fixed-transparent">'+'_';
		selectionBoxContent[isRec] += '</span></td>';  
	  ...
	  for(var j=0;j<x.length;j++){
		if(x[j].charCodeAt(0)>=48 && x[j].charCodeAt(0)<=57)
  ...
  BUSC[0] = selectionBoxContent[0].slice();
  BUSC[1] = selectionBoxContent[1].slice();
  $('#selectionBox').append(selectionBoxContent[0]);
  $('#selectionBox').append(selectionBoxContent[1]);
...
function confirmUserSelection() {
  ...
  //$("<div class='bubbledRight'>"+ humanSentenceSelection[selectionFlag][1] + "</div>").appendTo("#dialogueContentSection");
