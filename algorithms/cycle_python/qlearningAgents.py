		...
		for t in range(0,self.numOfTurn):
			for level in range(0,self.numOfLevelStatus):
				for worst in range(0,self.unitNum):
					for a in util.turnIndex2action('b','one_tree',t):
						state = (t,level,worst)
						key = (state,a)
						self.qVal[key] = 0.0
						self.numofQ += 1
...
#	@profile	
	...
	def getValue(self, state):
		value = 0.0
		actions = util.turnIndex2action('b','one_tree',state.dialogTurnIndex)
		if actions:
			Qvalues = []
			for action in actions:
				Qvalues.append(self.getQValue(state, action))
	...
	def getAction(self, state):
		# Pick Action
		legalActions = util.turnIndex2action('b','one_tree',state.dialogTurnIndex)
		action = None
		if legalActions:
			if util.flipCoin(self.epsilon): # if prob < epsilon 
				action = random.choice(legalActions)
