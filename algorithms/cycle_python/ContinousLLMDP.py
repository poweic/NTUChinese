...
class LLMDP(mdp.MarkovDecisionProcess):
	...
	def getPossibleActions(self, state):
		...
		"""
		if state.dialogTurnIndex == util.returnConvertedIndexListCount('b','one_tree'):
			return () # return () enables agent to make sure episode ends
		else:
			return tuple( util.turnIndex2action('b','one_tree',state.dialogTurnIndex) )
	...
	def getStates(self):
		...
		"""
		totalConvertedIndexNum = util.returnConvertedIndexListCount('b','one_tree') - 1 ## minus 1 for terminal
		turns = []
	...
	def getReward(self, state, action, nextState, userData):
		...
		if state.dialogTurnIndex == util.returnConvertedIndexListCount('b','one_tree') - 1:
			totalAverage = userData._phoneAverage+userData._toneAverage
...
#			print 
			...
			badReward = 0.0
			badLexNum = 0
			randomIFandTone = util.random_B_IFandTonecount_one_tree()
			max1 = util.max_B_IFandTonecount_one_tree()
			min1 = util.min_B_IFandTonecount_one_tree()
...
#			print  reward, AOP, explore
	...
	def isTerminal(self, state):
		...
		return state.dialogTurnIndex == util.returnConvertedIndexListCount('b','one_tree') - 1
...
#		print self.ntu_chinese_world.dialogInformation.node[nextDialogState]['content']	
		...
		linetone = self.ntu_chinese_world.dialogInformation.node[nextDialogState]['nodeTone']
		...
		convertedDialogIndex = util.returnConvertedIndex('b',nextDialogState,'one_tree')
